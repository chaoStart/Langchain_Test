...
# 用户查询
query_text = "我想知道8月人工成本表的企业人工成本总额的本年实发数"
...
for v in location_sheet_info.values():
    for each_row in row_results:
        if v["table_name"] == each_row["table_name"]:  # 说明sheet_name中匹配到了高分工作表中table_name
            ...
            merge_all_row_list.append({v["table_name"]:
                                           {
                                            "current_score": each_row["score"],
                                            "row_name": [each_row["row_name"].split('\n')[0]]
                                           }
                                       })
# 存储所有列数据
...
for v in location_sheet_info.values():
    ...
    if len(column_results) != 0:
        for each_column in column_results:
            if v["table_name"] == each_column["table_name"]:  # 说明sheet_name中匹配到了高分工作表中table_name
                ...
                merge_all_column_list.append({v["table_name"]:
                                                  {
                                                      "current_score": each_column["score"],
